...
head(cancer_sample)
```
has parameters of the cancer, such as radius, perimeter, area, etc. (32 variables)


```{r}
head(apt_buildings)
```
has presence of balconies, various amenities, fire alarm, storage room, number of elevators, year built (37 variables)  


```{r}
head(vancouver_trees)
```
has genus name, species name, cultivar name, height range, date planted (20 variables)
...
I will choose apt_buildings, since it has a nice variety of variables that can be used to do interesting data analysis regarding how apartments built in different time periods have different standards.
...
<!-------------------------- Start your work below ---------------------------->
I want to explore the relationship between the year of the apartment built with other variables such as number of storeys, number of elevators, presence of fire alarms, etc.
<!----------------------------------------------------------------------------->
...
First of all, we will plot the distribution of the year that the apartments were built, since this is our variable of interest (1.) This allows us to see if there is a meaningful spread of the year built variable, since it would be boring and meaningless for the data analysis if most apartments were built in a specific year.
...
The distribution of the years that the apartment were built seems decent enough for us to continue with the research question. However, it is possible that there may be some missing (NA) values for the year built variable. We can investigate that next and since we're doing it anyways, we will check it for all variables, in order to figure out if we want to maybe avoid using some variables with too many missing values (3.)
...
From this graph, it looks like we should avoid using the variables "prop_management_company_name" and "amenities" for our analysis, since they have a decent amount of NA values. Now, we will try looking at the relationship between year built and the number of elevators. (5.)
...
From the scatterplot above, we can see that there is a slight increase after 1950 for the number of elevators.
...
<!--- *****START HERE***** --->
The overarching research topic for my data analysis will be: Are newer apartments better?

In order to investigate this, I will look at 4 research questions that look at the relationship between the year that the apartment was built vs. variables that could potentially indicate a "better" apartment:

1. Explore the relationship between the year of the apartment built vs. number of elevators  
2. Explore the relationship between the year of the apartment built vs. presence of fire alarm  

3. Explore the relationship between the year of the apartment built vs. number of storeys (I personally think taller apartments are cooler)
 
4. Explore the relationship between the year of the apartment built vs. number of accessible parking spaces  
